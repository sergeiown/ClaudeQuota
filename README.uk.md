...
<picture>
  ...
  <img src="docs/popup-light.png" width="300" alt="Попап ClaudeQuota">
</picture>

Закріпіть попап (кнопка справа зверху), щоб він не закривався при втраті фокусу, і натисніть Minimize (з'являється поруч із Pin після закріплення), щоб згорнути його лише до двох шкал і часу скидання - легша, більш прозора індикація для спостереження за швидкими змінами використання під час тривалої роботи, яка не заважає.
...
<picture>
  ...
  <img src="docs/tray-menu-light.png" width="260" alt="Меню трею ClaudeQuota">
</picture>
...
Також ви отримаєте сповіщення Windows першого разу, коли використання вікна перевищить 51%, 81%, 99% і 100% - останнє також повідомляє, коли це вікно скинеться. Кожне з іконкою застосунку та мініатюрою відповідної шкали, клік по ньому відкриває попап. Нічого не спрацьовує заднім числом: використання, що вже перевищило порог на момент запуску застосунку, не викликає сповіщення про нього.
...
І все - решту ClaudeQuota робить сам. Йому потрібен авторизований Claude Code CLI за лаштунками (термінальний інструмент, окремий від Claude Desktop і claude.ai - вкладка **Code** в самому Claude Desktop виглядає схоже, але не створює логін, який читає ClaudeQuota), але відкривати термінал чи щось вводити самому не потрібно ніколи:

- **CLI взагалі відсутній?** Попап сам відкриється і запропонує встановити - один клік, без термінала. Використовує `winget` - вбудований у Windows 10 (1809+) і Windows 11 інсталятор, який є практично на будь-якій оновленій машині.
- **CLI встановлений, але не авторизований (чи логін протух)?** ClaudeQuota сам авторизується - вкладка браузера відкриється сама, без кліку. Просто увійдіть тим самим акаунтом, яким користуєтесь у Claude Desktop або claude.ai.

Вкладка браузера для входу виглядає так, і її можна закрити, коли вона це підтвердить:

<img src="docs/setup-browser-success.png" width="380" alt="Сторінка успішного входу Claude Code">

> Іконки в треї теж ніде не видно? Windows інколи ховає нові іконки - перевірте стрілочку **^** біля годинника. Якщо попап колись покаже кнопку "Log in" чи "Install Claude CLI" - просто натисніть, далі ClaudeQuota зробить усе сам.
